...
class PriceTable extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    for (var element in currentPrice) {
      ...
      mapPrice[element.name]!.add(
        PriceModel(buy: element.buy, sell: element.sell)
      );
    }
    ...
    return DataTable(
      ...
      headingRowColor: MaterialStateColor.resolveWith(
        (states) => AppColor.primaryGold),
      border: TableBorder.symmetric(
        outside: const BorderSide(width: 0.3, color: AppColor.border)
      ),
